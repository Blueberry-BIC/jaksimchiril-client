...
class CertifyStatusActivity : AppCompatActivity() {
    ...
    private var auth_method:Int=1 //챌린지가 어떤 인증 방식인지
...
//                    plusSuccessCount()
         ...
         */
            ...
            Log.e("태그, 액션인증 갓다가 인증현황옴", "challId: " + challId)
...
//        binding.textView9.text = myCertifyCount.toInt().toString() + "회"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.TimeTextView.text = "종료일: $endDate"

        //현재 챌린지 정보 가져와서 참가자 이름, 인증횟수들 텍스트뷰에 채우기
        getChallInfo()

        //인증하기 버튼 클릭시 -> 액션, 깃허브, 이미지, 시사뉴스, 걸음수 인증 -> 5개중 한개 페이지 이동
        binding.btnChallCertify.setOnClickListener {
            when (auth_method) {
                1 -> {
                    val intent = Intent(this@CertifyStatusActivity, CameraCertifyActivity::class.java)
                    intent.putExtra("challId", challId)
                    activityResultLauncher.launch(intent)
                }
                2 -> {
                    val intent = Intent(this@CertifyStatusActivity, GithubCertifyActivity::class.java)
                    intent.putExtra("challId", challId)
                    activityResultLauncher.launch(intent)
                }
                3 -> {
                    val intent = Intent(this@CertifyStatusActivity, ActionCertifyActivity::class.java)
                    intent.putExtra("challId", challId)
                    activityResultLauncher.launch(intent)
                }
                4 -> {
                    val intent = Intent(this@CertifyStatusActivity, WalkingCertifyActivity::class.java)
                    intent.putExtra("challId", challId)
                    activityResultLauncher.launch(intent)
                }
                5 -> {
                    val intent = Intent(this@CertifyStatusActivity, NewsCertifyActivity::class.java)
                    intent.putExtra("challId", challId)
                    activityResultLauncher.launch(intent)
                }
            }
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    val userList = jsonObject.getJSONArray("user_list")
                     auth_method = jsonObject.getInt("auth_method")
                    ...
                    for (i in 0 until userList.length()) {
                        ...
                        try {
                            ...
                            val username = CertifyData(
                                "참가자${num + 1}\n(${userid}..)",
                                certifyCount.toInt().toString() + "회 인증",
                                R.drawable.bic_logo
                            )
